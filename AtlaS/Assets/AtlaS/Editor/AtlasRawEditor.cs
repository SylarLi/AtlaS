...
namespace AtlaS
{
    ...
    public class AtlasRawEditor : Editor
    {
        ...
        {
            mPreviewMat = mPreviewMat ?? new Material(Shader.Find("AtlaS/Default"));
            mPreviewMat.SetTexture("_MainTex", mainTex);
            mPreviewMat.SetTexture("_AlphaTex", alphaTex);
            if (alphaTex != null)
                mPreviewMat.EnableKeyword("ATLAS_UI_ALPHASPLIT");
            else
                mPreviewMat.DisableKeyword("ATLAS_UI_ALPHASPLIT");
